...
router.route("/register").post(async(req,res)=>{
  ...
  try{
    ...
    if(userExits){
      ...
      const user = new User({name,email,phone,work,password,cpassword,dob});
...
router.route("/transporter/update/:id").post(async(req, res)=>{
  ...
  if(!name|| !pickupcity|| !dropcity){
    return res.status(422).json({error : "Plz fill the field properly"});
  }
  try{
    const r = await Truck.updateOne({number : number},{$set:{name,pickupcity,dropcity,capacitty,price}});
    console.log(r);
...
router.route('/booking').post(async(req,res)=>{
    try{
      const {user, pickupcity, dropcity, date, weight,  adminemail,truckid,typeofgoods,price,Address1,Address2} = req.body;
      const status = false;
...
router.route('/bookingRequests').post(async(req,res)=>{
    try{
      ...
      const result = await Booking.find({transemail : transemail,transconfirm:false});
      if(result.length<=0){
